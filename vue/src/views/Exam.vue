...
<!--      <el-input style="width: 200px" placeholder="请输入" suffix-icon="el-icon-position" class="ml-5" v-model="address"></el-input>-->
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="room" label="考试地点"></el-table-column>
      ...
      <el-table-column prop="teacher" label="选择试卷">
        <template v-slot="scope">
          <el-button type="success" @click="handleSelect(scope.row)">选择 <i class="el-icon-edit"></i></el-button>
        </template>
      </el-table-column>
      <el-table-column prop="teacher" label="查看成绩">
        <template v-slot="scope">
          <el-button type="success" @click="viewGrade(scope.row.id)">查看 <i class="el-icon-edit"></i></el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="40%" :close-on-click-modal="false">
      <el-form label-width="120px" size="small" style="width: 80%; margin: 0 auto">
        ...
        <el-form-item label="考试时间">
          <el-date-picker v-model="form.time" type="datetime" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
    ...
    <el-dialog title="选择试卷" :visible.sync="dialogFormVisible1" width="40%" :close-on-click-modal="false">
      <el-select v-model="form1.paperId" placeholder="请选择">
        <el-option
...
export default {
  ...
  methods: {
    ...
    handleSelect(row){
      ...
      this.form1={examId:row.id,paperId: row.paperId}

      this.request.get("/paper").then(res => {
        this.papers=res.data.filter(v=>v.courseId===row.courseId)
      })
      this.request.get("/examPaper/exam/"+row.id).then(res => {
        if (res.code==='200'){
